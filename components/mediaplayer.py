...
''')
    ...
    if not running:
        ...
        if args.artist:
            print(fmt_field('Artiste',
                metadata['xesam:artist'][0] if 'xesam:artist' in metadata else 'Inconnu',
                args.conkyalignr, args.width))
        if args.title:
            print(fmt_field('Titre',
                metadata['xesam:title'] if 'xesam:title' in metadata else 'Inconnu', args.conkyalignr, args.width))
        if args.album:
            print(fmt_field('Album',
                metadata['xesam:album'] if 'xesam:album' in metadata else 'Inconnu', args.conkyalignr, args.width))
        if args.genre:
            print(fmt_field('Genre',
                metadata['xesam:genre'][0] if 'xesam:genre' in metadata else 'Inconnu', args.conkyalignr, args.width))
        if args.year:
            print(fmt_field('Année',
                metadata['xesam:contentCreated'][:3] if 'xesam:contentCreated' in metadata else 'Unknown',
                args.conkyalignr, args.width))
        if args.track:
            print(fmt_field('Piste',
                metadata['xesam:trackNumber'] if 'xesam:trackNumber' in metadata else 'Inconnue',
                args.conkyalignr, args.width))
        if args.elapsed:
            print(fmt_field('Temps écoulé', fmt_time(player.Get('org.mpris.MediaPlayer2.Player', 'Position',
                dbus_interface='org.freedesktop.DBus.Properties')), args.conkyalignr, args.width))
        if args.length:
            print(fmt_field('Durée',
                fmt_time(metadata['mpris:length']) if 'mpris:length' in metadata else 'Inconnue',
                args.conkyalignr, args.width))
        if args.progress:
